 ...
 */
@ScriptManifest(author = "El Maestro", category = Category.PRAYER, description = "Trains prayer by using any bone on an altar at premium area.", name = "ELPrayer", version = 1.0, servers = {"PKHonor"})
public class Core extends Script implements Paintable, MessageListener {
    ...
    public void messageReceived(MessageEvent message) {
        if (message.getMessage().contains("You received")) {
            Variables.setPkpEarned(Variables.getBonesOffered() + 1);
        }
